...
int main()
{
    ...
    char userIn = ' ';                                          // var to get input from the user to count the letters and determine the language
    ...
    int temporarySortedArray[totalMcbethFiles + 1][LIMIT];      // 2d array to store the lettersCountArr's numbers and to store the counted number of userIn
    string langTextFile[totalMcbethFiles] = {"MacbethEnglish.txt","MacbethFinnish.txt","MacbethFrench.txt","MacbethGerman.txt","MacbethHungarian.txt","MacbethItalian.txt","MacbethPortuguese.txt","MacbethSpanish.txt"};
    ...
    for (int fileloop=0;fileloop<totalMcbethFiles;fileloop++){
        ...
        for (i=0;i<LIMIT;i++){                                              // storing counted occurences of letters in each row of on one iteration of j
            if(lettersArray[i] != 0){
        ...
        inputFileStream.close();                                            //closing file
    }                                                                       // end of looping through 8 files;
    
    cin >> userChoice;                                                      // getting input from user to display and work accroding to menu options above:
    
    if (userChoice == 0){                                                   // exiting for input 0
        exit(0);
    }
    
    if(userChoice == 1){                                                     // displaying letter frequency counts
    ...
    if(userChoice >= 2){                                                     // if user chooses menu option 2 or more:
        displayLettersFrequencyCount(lettersCountArr, LIMIT, totalMcbethFiles);
        
        frequencyFunction(lettersCountArr, alphaArray);                      // sorting letters acorrding to frequency
        
        displayLettersFrequencyOrder(alphaArray, LIMIT, totalMcbethFiles);   // function called to display the letter frequency order
        ...
        if(userChoice >= 3){
            ...
            if ( userChoice ==4)
            {
                ...
                for(int languages=0; languages<totalMcbethFiles; languages++){
                    ...
                    for(int userColRow=0; userColRow<LIMIT; userColRow++){
                        userChar = sortedUserAlphaArray[totalMcbethFiles][userColRow];  // storing each letter of user column particular index to compare
                        
                        // loop for each letter in a column to compare
                        for (int langColRow = 0; langColRow < LIMIT; langColRow++){
                            
                            langChar = sortedUserAlphaArray[languages][langColRow];     // storing each letter of particular index to compare
                            
                            if (userChar == langChar){
                                difference = abs(userColRow - langColRow);              // finding absolute diffrence between columns same letters

                                if (difference <=maxThreshold){
                                    difference =0;                                      // if the difference less the 4 then assigng it to 0 which will
                                                                                        // help keeping the diffrence count low and helping estimating the language
                                }
                            }
                            else{                                                       // if diffrent letter then continuing to next letter
                                continue;
                            }
                            wholeColDifference += difference;                           // adding all diffrences of the column into one variable
                        } // end of third for loop
                    } // end of second for loop
                    if (wholeColDifference < trueMin){                                  // assigning the trueMin to the most minimum diffrence of all languages
                        trueMin = wholeColDifference;
                    }
                    differenceArray[languages] = wholeColDifference;                    // storing the each language differnce to this array
                } // end of first for loop
                
                // string array containg the language names
                string languageName[totalMcbethFiles]={"Language is English","Language is Finnish","Language is French","Language is German","Language is Hungarian","Language is Italian","Language is Portuguese","Language is Spanish"};
            ...
            } // if statement ends of input 4
        } // if statement ends of input  3
    } // if statement ends of input 2
